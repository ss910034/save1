...
- (void)initLayer {
    ...
    _itemTextColor = [Constants getDefaultTextColor];
    _itemFontSize = 14.0;
...
-(void)didTap : (UIGestureRecognizer *)gesture {
    ...
    if(!isCollapsed) {
        CGFloat height = (CGFloat)(_items.count > Constants.getProductCategoryDic.count ? _itemHeight*Constants.getProductCategoryDic.count : _itemHeight * (double)(_items.count));
        
        tblView.layer.zPosition = 1;
        [tblView removeFromSuperview];
        tblView.layer.borderColor = [[Constants getDefaultLineColor] CGColor];
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            if(_DirectionDown)
                tblView.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y + self.frame.size.height+5-5, self.frame.size.width, height);
            else
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([label.text isEqualToString:@"顯示全部"]) {
        label.text = @"    顯示全部";
    }
